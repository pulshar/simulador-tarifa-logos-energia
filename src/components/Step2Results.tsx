...
import { CheckCircle, TrendingDown, MoveRight } from 'lucide-react';
...
export function Step2Results({ result, importeActual, onReset }: Props) {
  ...
  const ahorro = importeActualNum ? importeActualNum - currentResult.total : null;
  const isAhorro = ahorro !== null && ahorro > 0;

  const Row = ({ label, value, delay = 0 }: { label: string, value: number, delay?: number }) => (
    <motion.div
    ...
    >
      <div className="card-premium p-8 md:p-10 mb-8 relative overflow-hidden">
        ...
        <div className="flex flex-col gap-1 mb-8">
          <Row label="Término de potencia" value={currentResult.potencia} delay={0.1} />
          <Row label="Término de energía" value={currentResult.energia} delay={0.2} />
          <Row label="Otros conceptos" value={currentResult.otros} delay={0.3} />
        ...
        >
          <p className="text-ink-soft text-xs mb-1 uppercase tracking-[0.12em] font-mono font-medium">Total estimado</p>
          ...
          {isAhorro && (
            <div className="mt-4 inline-flex items-center gap-1.5 bg-accent-soft text-brand-ink px-3 py-1.5 rounded-full text-xs font-mono font-medium tracking-tight shadow-sm">
              ...
              <span>Ahorrarías <AnimatedNumber value={ahorro} suffix=" €" /> respecto a tu factura actual</span>
            </div>
          )}
          {!importeActualNum && (
            <div className="mt-4 text-ink-soft text-xs font-mono font-medium tracking-tight">
              Con Logos Energía pagarías aproximadamente <AnimatedNumber value={currentResult.total} suffix=" €" />
            </div>
          )}
        </motion.div>
        ...
        <p className="text-[11px] text-ink-mute text-center mt-10 leading-relaxed font-sans">
          Estimación realizada según los precios vigentes de Logos Energía. Los precios utilizados para el cálculo de la energía en la tarifa variable se basa en la cotización de OMIP del próximo año móvil. Estos precios pueden fluctuar en función del mercado.
        </p>
      </div>
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            <span>Contratar tarifa {activeTab === 'clasica' ? 'clásica' : 'variable'}</span>
